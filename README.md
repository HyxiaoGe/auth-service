...
[完整接入指南](docs/INTEGRATION_GUIDE.md) 开始；简版清单见
[ONBOARDING](docs/ONBOARDING.md)，端点与 Token 契约见
...
> `auth-client-web` 是无 UI 的浏览器认证 SDK，不提供登录弹窗、按钮或表单样式；应用负责
> 实现登录界面，并调用 SDK 与 Auth Service 的邮箱交互端点。
